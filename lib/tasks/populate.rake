namespace :db do
  ...
  task :populate => :environment do
    ...
    # -----------------------    
    # Step 1: add some animal types to work with (small set for now...)
    ...
    puts "Animal count: #{animal_ids.count}"
    
    # -----------------------
    # Step 2: add 240 owners to the system and associated pets
    240.times do 
      owner = Owner.new
    ...
    # Step 3: Give each owner 1 to 3 pets
    all_owner_ids.each do |oid| 
      ...
      num_pets.times do 
        ...
        pet.dob = (30..4400).to_a.sample.days.ago
        # now save the object
        pet.save!
      end
    end  
    ...
    # Step 4: add between 1 to 15 visits for each pet
    all_pets.each do |pet|
      ...
      num_visits.times do
        ...
        visit.date = Faker::Time.between(from: pet.dob, to: Date.today).to_date
